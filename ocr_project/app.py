...
with open(r'C:\Users\rohitha\Downloads\ocr_project\finalized_model.sav', 'rb') as f:
    model = pickle.load(f)
...
if uploaded_file is not None:
    ...
    # Create a list to store the suspicious words
    suspicious_words = []
    ...
    for contour in contours:
        ...
        if aspect_ratio > 1.5 and aspect_ratio < 10:
            ...
            if mean_color[0] < 100 or mean_color[0] > 200:
                ...
                suspicious_words.append(roi_thresh.flatten())
    ...
    if st.button('Predict Output'):
        # Check if any suspicious word was detected
        if len(suspicious_words) >= 1:
            # Use the random forest model to predict if the image contains a suspicious word or not
            predictions = model.predict(suspicious_words)
            ...
            st.write('No suspicious words found in the image.')
